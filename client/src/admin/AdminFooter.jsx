...
export default function AdminFooter() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      brandText: Yup.string().required("Brand name is required"),
      contact: Yup.object({
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-2xl p-8">
      <h2 className="text-3xl font-bold mb-8">Footer Settings</h2>
      ...
      <form onSubmit={formik.handleSubmit} className="space-y-12">
              ...
              <div className="flex gap-3 flex-wrap">
                <button
                  ...
                  className={`px-4 py-1 rounded ${
                    l.isActive ? "bg-green-600 text-white" : "bg-gray-300"
                  }`}
                >
        ...
        >
          {saving ? <Loader2 className="animate-spin mx-auto" /> : "Save Footer"}
        </button>
      </form>
    </div>
  );
